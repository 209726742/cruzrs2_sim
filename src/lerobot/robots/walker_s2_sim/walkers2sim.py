...
class WalkerS2sim(Robot):
    ...
    def __init__(self, config: WalkerS2Config | dict | None = None, teleop: Any = None):
        ...
        # 头部相机可视化（headless 模式下禁用，避免 OpenCV/Qt 在无 DISPLAY 环境崩溃）
        head_viz_enabled = getattr(self.config, "head_viz_enabled", False) and not getattr(
            self.config, "headless", False
        )
        self._head_visualizer = HeadStereoVisualizer(
            enabled=head_viz_enabled,
