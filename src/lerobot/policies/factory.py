...
def make_policy(
    ...
    # Only truncate when the selected policy has an explicit architectural limit.
    # Dataset-specific dimensions below that limit must remain unchanged.
    max_state_dim = getattr(cfg, "max_state_dim", None)
    if (
        OBS_STATE in features
        and max_state_dim is not None
        and features[OBS_STATE].shape[0] > max_state_dim
    ):
