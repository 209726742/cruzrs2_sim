...
class PiGemmaModel(GemmaModel):  # type: ignore[misc]
    ...
    def forward(
        ...
        **kwargs,
        ...
        # transformers >= 5.x 的 create_causal_mask 移除了 cache_position 参数（内部自动计算）
        _mask_kwargs = dict(
            config=self.config,
        ...
        if "cache_position" in _inspect.signature(create_causal_mask).parameters:
            _mask_kwargs["cache_position"] = cache_position
