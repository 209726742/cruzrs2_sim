...
# Optional compatibility override for datasets that intentionally expose more
# state than a policy should consume. Normal task datasets use their declared
# state dimension without an implicit 20-dim truncation.
_STATE_N_DIMS_OVERRIDE = os.environ.get("LEROBOT_STATE_N_DIMS")
...
def train(cfg: TrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    state_n_dims = (
        ...
        else dataset_state_dims
    )
    if state_n_dims > dataset_state_dims:
        raise ValueError(
            f"LEROBOT_STATE_N_DIMS={state_n_dims} exceeds dataset state dimension {dataset_state_dims}"
        )
    max_state_dim = getattr(policy.config, "max_state_dim", state_n_dims)
    if state_n_dims > max_state_dim:
        raise ValueError(
            f"state dimension {state_n_dims} exceeds policy max_state_dim={max_state_dim}"
        )

    truncated_stats = slice_stats_for_state(dataset.meta.stats, n_dims=state_n_dims)
