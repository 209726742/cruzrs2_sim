...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    try:
        ...
        auto_start = cfg.policy is not None and (
            is_headless() or getattr(cfg.robot, "headless", False)
        )
        if auto_start:
            logging.info("Headless policy inference: auto-starting recording (Enter key not available).")
            events["start_record"] = True
            logging.info("Warming up simulation render pipeline before recording...")
            for _ in range(30):
        ...
        # Print the real error BEFORE the finally block closes Isaac Sim,
        # because kit.close() (fastShutdown) kills the process and swallows tracebacks.
        import traceback
        logging.error("Exception during recording:\n%s", traceback.format_exc())
        raise
    finally:
