...
> 整理结果：原顶层 71 个执行/测试文件已完成可逆物理分类，没有永久删除源码。

## 先看这里

- 当前正式采集代码只在 `collection/` 与 `core/`；常用入口从 `collection/` 运行。
- `diagnostics/` 只做 NOREC 回归和几何探针，不发布训练数据。
- `tests/` 是当前链的 15 个测试文件，现有测试总数为 65。
- `archive/` 保存旧 ECU、RL/DAgger 和历史专家，不得与当前双物料数据混用；归档代码不承诺可原地运行。
- 当前仍受“仅可见 1 张 GPU + 严格任务代表 seed 0/4”阻塞，禁止正式多卡采集；详见[主进展文档](../../双物料线_pi05训练前评估与数据采集建议.md)。
...
├── core/             # 当前共享契约、控制、质量与对象工具（10）
├── diagnostics/      # 当前 NOREC/几何诊断（3）
├── tests/            # 当前回归测试文件（15，合计 65 项测试）
└── archive/
    ├── ecu/          # 旧 ECU/纸箱流水线（18）
...
- 归档是保留，不是当前支持：文件仍存在，但内部旧路径、旧相机和旧数据契约没有纳入本轮 65 项测试。
