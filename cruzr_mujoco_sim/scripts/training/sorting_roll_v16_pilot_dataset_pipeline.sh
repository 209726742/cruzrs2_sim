...
"$ISAAC_PY" - "$DATASET_V21" "$DATASET_V30" "$REPO_ID" \
  "$BUILD_REPORT" "$AUDIT_REPORT" "$SAMPLING_REPORT" "$READINESS" <<'PY'
import json
from pathlib import Path
import sys

v21, v30, repo_id, build_path, audit_path, sampling_path, output_path = sys.argv[1:]
